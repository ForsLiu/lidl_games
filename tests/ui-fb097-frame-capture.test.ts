...
describe('fb097: dev-profile frame capture', () => {
  ...
  it('captures the expected frame count over the fixed interval and downloads one ZIP', async () => {
    ...
    const createObjectURLCall = createObjectURL.mock.calls[0];
    if (!createObjectURLCall) throw new Error('expected createObjectURL to have been called');
    const zipBlob = createObjectURLCall[0] as Blob;
