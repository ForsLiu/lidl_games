...
describe('fb034 practice tool: max all towers', () => {
  it('raises a freshly built tower straight to its final tier, free, and is a no-op outside practice', () => {
    ...
    // fb153 (World now generates against all four base `GATES`, not a stale
    // 3-gate slice): (5,5) is real generated terrain here (this run is
    // deliberately non-practice, to prove the tool is a no-op outside
    // practice) and is no longer open ground at seed 1 — (4,7) is.
    const off = new Run({ ...cfg(), policy: 'none' });
    buildAt(off.world, 4, 7);
    const sOff = off.world.structureAt(4, 7)!;
