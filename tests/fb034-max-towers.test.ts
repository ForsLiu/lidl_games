...
describe('fb034 practice tool: max all towers', () => {
  it('raises a freshly built tower straight to its final tier, free, and is a no-op outside practice', () => {
    ...
    // fb153b/fb153 (56x32 grid, real gate list): (5,5) is real generated
    // terrain here (this run is deliberately non-practice, to prove the tool
    // is a no-op outside practice) and is no longer open ground at seed 1 —
    // (6,7) is.
    const off = new Run({ ...cfg(), policy: 'none' });
    buildAt(off.world, 6, 7);
    const sOff = off.world.structureAt(6, 7)!;
