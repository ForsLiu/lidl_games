 ...
 * `not a soul tower: ...` error without touching argv, which
 * `tools/m20d-price-probe.ts` hardcodes to `venom_spore`: instead, the
 * tower's `key` is renamed in the scratch `data/towers.json` before running
 * the probe, which drops `venom_spore` out of `a4probe.ts`'s `SOUL_TOWERS`
 * (post-p2e: the towers with a non-null attack, keyed by `t.key`) — the
 * same effect as an unknown key, reached through the one lever this test
 * actually controls. (The lane's original lever, nulling `venom_spore.soul`,
 * died with the soul field at the 2026-08-28 merge; a renamed key loads
 * clean because no `/data` file cross-references it and the loader does not
 * check `/src` string-literal key references — q7's documented E1 gap.)
 */
 ...
 */
function breakVenomSporeKey(dir: string): void {
  const p = towersPath(dir);
  const before = readFileSync(p, 'utf8');
  const anchor = '"key": "venom_spore"';
  const count = before.split(anchor).length - 1;
  if (count !== 1) {
    throw new Error(`breakVenomSporeKey: expected exactly one occurrence of ${JSON.stringify(anchor)}, found ${count}`);
  }
  writeFileSync(p, before.replace(anchor, '"key": "venom_spore_retired"'));
}
