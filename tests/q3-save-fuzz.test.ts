...
describe('q3 save fuzz: the corpus is not degenerate', () => {
  ...
  it.each(FAMILIES)('family %s still changes what loads', (family: Family) => {
    ...
    // byFamily is keyed by every Family (line 83 pins the key set to FAMILIES
    // exactly), so the just-iterated `family` is always present.
    if (s === undefined) throw new Error(`missing byFamily entry for ${family}`);
