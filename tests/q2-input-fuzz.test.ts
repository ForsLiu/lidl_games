 ...
 *
 *  - `fuzzPhase` holds the world in one of the eight `Phase` values and fires
...
describe('q2 input fuzz', () => {
  ...
  it('generates every member of the Command union', () => {
    // The same guard as the phase one above, for the same reason: a 13th
  ...
  it('has an invariant scan that actually fires', () => {
    ...
    const probes: { name: string; poison: () => void; undo: () => void }[] = [
      ...
      {
        name: 'soulLevels',
        poison: () => { w.soulLevels.__probe = { level: NaN, damageBonus: NaN }; },
        undo: () => { delete w.soulLevels.__probe; },
      },
      (() => {
  ...
  it('survives whole runs with random commands in the tick input', () => {
    for (const seed of [1, 2, 3]) {
      ...
      // The run was played, not skipped through. With the practice tool live,
      // `god`/`invuln`/`fast_forward` end a run in ~6.5k ticks with the damage
      // and defeat paths switched off; a real run is several times that, and
      // asserting the floor is what keeps this case honest.
      expect(r.ticks, `seed ${seed} ended in ${r.ticks} ticks`).toBeGreaterThan(10000);
    }
  });
  ...
  it('survives whole runs with the practice tool live', () => {
    ...
    // twelve is a guaranteed no-op. These runs are the ones that reach it.
    for (const seed of [1, 2, 3]) {
