 ...
 * pre-checked guards before a lookup, `.charAt(0)` instead of `s[0]`), then 7
 * more (`src/sim/rng.ts`, `src/ui/character-panel.ts`, `src/render/canvas.ts`,
 * `src/sim/classes.ts`, `src/render/theme.ts`, `src/sim/sundering.ts`,
 * `src/sim/terrain/config.ts`) in a third session — see PROGRESS.md/
 * BACKLOG.md fb133 for the lists. The remaining 203 are exactly
...
const KNOWN_UNCHECKED_ACCESS_FILES: readonly string[] = [
  ...
  'tests/b033-boon-contrast.test.ts',
  'tests/b073-act1-alive-cap.test.ts',
  ...
  'tests/class-board-windows.test.ts',
  ...
  'tests/class-kit-liveness.test.ts',
  'tests/class-kit-power-reach.test.ts',
  'tests/class-kit-whiff.test.ts',
  ...
  'tests/equip-effectkey-reach.test.ts',
  ...
  'tests/fb019-training-grounds.test.ts',
  'tests/fb022-info-surfacing.test.ts',
  'tests/fb025-enemy-hp-bars.test.ts',
  'tests/fb027-selection-panels.test.ts',
  'tests/fb028-effect-text.test.ts',
  ...
  'tests/fb084-summon-cap-stat.test.ts',
  ...
  'tests/fb140-ci-workflow.test.ts',
  'tests/fb152-dot-tick-cadence.test.ts',
  'tests/fb153a-number-scale.test.ts',
  ...
  'tests/p3a-run-shape.test.ts',
  'tests/p3b-multi-summon.test.ts',
  'tests/p5c-milestone-specials.test.ts',
  'tests/p5d-projectile-damage-credit.test.ts',
  ...
  'tests/p8a-wave-content.test.ts',
