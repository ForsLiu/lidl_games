...
describe('fb065f — describeTerrain carries its gate list', () => {
  ...
  // Re-enabled by fb153 (main lane): `World`'s constructor (`src/sim/world.ts`)
  // now builds `this.gates` from the real `GATES`/`MODIFIER_GATES` exports
  // (`GATES.slice()`, then `MODIFIER_GATES[0]` when the `gate` modifier is
  // active) instead of its own stale `GATES.slice(0, 3)` copy plus a hand-typed
  // `south` literal at a dead (12, 19) tile. `w.gates` is therefore
  // `[west, north, east, south, south2]` under the modifier — five gates, not
  // four — with `south2` at `MODIFIER_GATES`'s real (3, 31).
  it('describes a live Fourth Gate run correctly — the case that motivated it', () => {
    ...
    // `gate` modifier plays five gates; before fb065f its repro printed three
