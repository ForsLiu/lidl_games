...
const RECORDED: Record<string, { actual: string; target: string; met: boolean }> = {
  classes: { actual: '3', target: '11', met: false },
  ...
  quests: { actual: '8', target: '8-12', met: true },
...
describe('content census (SPEC-FINAL §13)', () => {
  ...
  it('the met/unmet split matches the P-phase audit: exactly classes/equipment/waves are short', () => {
    const unmet = rows.filter((r) => !r.met).map((r) => r.key).sort();
    expect(unmet).toEqual(['classes', 'equipment', 'waves'].sort());
  });
