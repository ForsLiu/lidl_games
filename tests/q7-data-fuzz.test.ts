 ...
 *     the only reason a 4,700-case census costs 14 s instead of 50.
...
const holders = vi.hoisted(() => {
  const names = [
    'affinity', 'boons', 'classes', 'damagetypes', 'dev', 'enemies', 'modifiers',
    'quests', 'relics', 'spawns', 'towers', 'tree', 'warden', 'waves', 'weapons',
  ];
...
vi.mock('../data/affinity.json', () => ({ default: holders.affinity }));
...
vi.mock('../data/weapons.json', () => ({ default: holders.weapons }));
...
describe('q7 — every field, every wrong shape', () => {
  ...
  it('writes nothing to /data', async () => {
    ...
    // the 4,775 mutations have already happened and a before/after pair taken
...
describe('q7 — cross-file references, row by row', () => {
  ...
  it('checks references one way only, so an unreferenced row can be renamed (E1)', async () => {
    const observed = await runCensusB();
    // The five primary-key fields that scored `partial`: some rows are pointed
...
describe('q7 — what accepted data does to a running game', () => {
  it('a renamed tower key loads clean and then crashes the engine (E1)', async () => {
    const r = await probe({
      name: 'palisade renamed',
      file: 'towers',
      edit: (root) => {
        const towers = (root as unknown as { towers: { key: string }[] }).towers;
        towers.find((t) => t.key === 'palisade')!.key = 'wall_thing';
      },
    });
    // `src/bots/policies.ts:216` does `towerByKey.get('palisade')!` and
    // `src/sim/upgrades.ts:119` / `src/sim/weapons.ts:339` compare `def.key ===
    // 'palisade'`. `palisade` is referenced by no other /data file, so the
    // integrity block never looks at it and the rename is accepted.
...
describe('q7 — filed defects (unskip with the fix)', () => {
  it.skip('E1 — the loader refuses a content key that /src names by string literal', async () => {
    ...
    root.towers.find((t) => t.key === 'palisade')!.key = 'wall_thing';
    const r = await load('towers', root as unknown as JsonValue);
    expect(r.outcome).toBe('rejected');
  });
