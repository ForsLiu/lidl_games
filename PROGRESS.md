...
- **2026-09-25 (scheduled terrain-lane routine) — BACKLOG-TERRAIN.md fb205
  ...
  **Deliberately not wired into the loader's `minCoreLegalFrac` check** —
  ...
  Q222). Ran `tools/sweep.ts` (engineer/hybrid: T3/12 seeds 25% win, T1/10
  ...
  **Left `[ ]`: the file's own "a scripted run reaches it, kills it and
  ...
  `/src/sim`) — refs: QUESTIONS Q220 item 10, Q222.
...
- **2026-09-22 (scheduled routine) — fb133 ratchet shrunk 5 → 4,
  ...
  `pick`/`shuffle`/`sample`, `src/sim/terrain/generate.ts`'s `scatter`):
  `map.kind[i] ?? TerrainKind.Normal` inline defaults at every
  bounds-checked terrain-kind read (`walkableFlood`, `thickMask`,
  `blockMask`, `corridorsOk`, `uncontestedHigh`, `gatesOpen`,
  `measureTerrain`), matching `?? -Infinity`/`?? -1` numeric defaults in
  `labelComponents`, and explicit `if (x === undefined) break/return`
  guards on queue-dequeue values used for further indexing
  (`walkableFlood`, `labelComponents`, `gateComponent`, `gatesConnected`).
  Does not touch `/data`. Verified: `npx tsc --noEmit -p
  tsconfig.unchecked.json` no longer flags the file, no new offenders
  (5 → 4); main `npx tsc --noEmit` clean; targeted `npx vitest run` (93
  tests across `fb133`, `terrain-generation`, `terrain-anchor-quality`,
  `terrain-describe`) green; `npm run test:fast` green (315 files passed /
  9 skipped, 324 total — the split has moved from unrelated suite growth
  since the last fb133 reading, not a regression); `npm run sim --seed 1
  --policy hybrid` endHash unchanged (`d6452f98`). code-reviewer APPROVE
  (one Minor: `gatesConnected`'s undefined-guard rests on an invariant
  across two independently-defaultable parameters, true at the sole call
  site but not type-enforced — addressed with a comment, not a behavior
  change). qa-playtester PASS — independently traced every guard against
  real production entry points (`gridTerrain()`, `describeTerrain()`, the
  generator, `overlay.ts` all enforce `map.kind.length === map.w*map.h`),
  fuzzed adversarial configs (1x1 grid, all-rock map, zero gates, no legal
  Core anchor) with no fallback ever firing; flagged the same
  `gatesConnected`/`gateComponent` `reachIn`/`gates`-pairing footgun as
  advisory-only (unreachable through any current call site). Full tier.
  **4 files remain, all `/src/sim`**: `enemies.ts`, `grid.ts`, `run.ts`,
  `world.ts`. — refs: BACKLOG.md fb133 Log.
- **2026-09-22 (scheduled routine) — fb133 ratchet shrunk 6 → 5,
...
- **2026-09-21 (scheduled routine) — fb133 ratchet shrunk 7 → 6.**
  ...
  calling it twice), `w.areas`/`w.gems` last-element reads, and an
  `enemies[0]` hoisted into a reused local. Does not touch `/src` or
  `/data`. Verified: `npx tsc --noEmit -p tsconfig.unchecked.json` no
  longer flags the file, no new offenders (7 → 6); main `npx tsc --noEmit`
  clean; targeted `npx vitest run` plus the ratchet test green (17 tests);
  ...
  dispatch. **6 files remain**: 5 `/src/sim` files (Full tier once picked
  up) and `tests/p2b-wielded-fire.test.ts` (the last test file, Light
  tier). — refs: BACKLOG.md fb133 Log.
- **2026-09-21 (scheduled routine) — fb133 ratchet shrunk 8 → 7.**
...
- **2026-09-21 (scheduled routine) — fb133 ratchet shrunk 20 → 18.**
  ...
  `before[i]`/`both[0]+both[1]` index; one `pair[0]`/`pair[1]` Enemy-array
  site fixed by destructuring the pair into two named consts instead of
  indexing) and `tests/fb044-tuner-per-field.test.ts` (an `nth<T>` helper,
  bulk-replacing every `doc.<collection>[0]`/`after.<collection>[0]`
  across towers/classes/cores/waves, a nested `.groups[0]` double-index,
  and a `fetchMock.mock.calls[0]` destructure). While fixing those two,
  ...
  `tests/p9c-tuner-ui.test.ts`, `tests/ui-fb139-bug-report-hotkey.test.ts`
  — each carrying the same latent unguarded `const [url, init] =
  fetchMock.mock.calls[0]` (or `applyPoisonMock.mock.calls[0]` in fb082)
  destructure. Confirmed genuinely reproducible (not tsc-subprocess
  flakiness) via repeated `git stash`/direct `tsc` runs, each state
  checked 3x for a stable file set before moving on; fixed all three with
  the same guard idiom (`const call = mock.calls[0]; if (!call) throw
  ...; const [url, init] = call;`). None of the 5 touch `/src/sim` or
  `/data`. Verified: `npx tsc --noEmit -p tsconfig.unchecked.json` file
  set matches the updated allowlist exactly, stable across 3 repeated
  runs (20 → 18); main `npx tsc --noEmit` clean; targeted `npx vitest
  run` on all 5 touched files plus the ratchet test itself green (60
  tests); `npm run test:fast` unchanged at 315 files / 4548 passed / 35
  skipped. Light tier (`[polish]`, no `/src`/`/data` touched) — no
  qa-playtester dispatch. — refs: BACKLOG.md fb133 Log.
- **2026-09-21 (scheduled routine) — fb133 ratchet shrunk 23 → 20.**
...
- **2026-09-21 (scheduled routine, latest) — fb133 ratchet shrunk 32 → 29.**
  ...
  seeding the running min/max instead of repeating the unguarded read; a
  local `nth<T>(arr, i)` throw-guard helper replacing every `maps[i]`/
  `maps[j]` indexed read in the pairwise diff-share double loop); `tests/
  class-active1-potency.test.ts` (a `firstEnemyKey()` helper matching the
  pattern already used in several sibling test files, replacing an unsafe
  `content.enemies.enemies[0].key` read; a throw-guard on `potencyCard()`'s
  `own[0]` return value; a local `nth<T>(arr, i)` helper replacing every
  unguarded `readings[...]`/`cost[...]`/`dmg[...]`/`plain[...]`/`added[0]`/
  `e.dots[0]`/`before[i]` indexed read across the file's damage-ladder
  assertions); `tests/equip-spec-ledger.ts` (the shared spec-ledger parsing
  module several other test files import, not a test file itself: a
  throw-guard on `src[i]` in `blankNonCode()`'s char scan, a `lineAt(j)`
  throw-guard closure replacing unguarded `ls[j]` reads throughout
  `blockBodyIn()`, a throw-guard on `hits[0]` before destructuring it,
  explicit undefined-checks on 5 regex capture groups in the `KILLS` table
  entry parser — none of these throw paths are reachable given the
  existing loop bounds/length checks, so parsing behavior is unchanged).
  ...
  `npx vitest run` on the 3 files' own tests plus every test file
  importing `equip-spec-ledger.ts` plus the ratchet test itself (262
  tests) green; `npm run test:fast` unchanged at 315 files / 4548 passed /
  35 skipped. 32 → **29 files remain** on the allowlist. code-reviewer
  APPROVE, no Critical/Major/Minor findings. Light tier (`[polish]`, no
  `/src`/`/data` touched) — no qa-playtester dispatch. — refs: BACKLOG.md
  fb133 Log.
- **2026-09-21 (scheduled routine, earlier) — fb133 ratchet shrunk 35 → 32.**
...
- **2026-09-20 (scheduled routine) — fb133 ratchet shrunk 84 → 74.**
  ...
  `tests/p-core-e-time-decay.test.ts`, `tests/render-fb060-dot-tick-numbers.test.ts`,
  `tests/render-fb116-terrain-rendering.test.ts`,
  `tests/ui-fb082-overlay-geometry.test.ts`, `tests/ui-fb096-save-slots.test.ts`,
  `tools/gate-audit.ts` — mostly throw-on-already-guaranteed-invariant guards
  and a couple of new helpers (`step(n)` in p-core-e, a shared `firstEnemyKey(w)`
  in render-fb060, a `hexChannel` helper in render-fb116) matching the
  established fb133 conventions; one real type fix (`ui-fb082`'s `cvVars`
  return type). Verified: `npx tsc --noEmit -p tsconfig.unchecked.json`
  clean on all 10, no new offenders; main `npx tsc --noEmit` clean; targeted
  `npx vitest run` green (284 tests); `npm run test:fast` green, unchanged
  at 315 files / 4548 passed / 35 skipped. code-reviewer APPROVE (one
  informational Minor/Nit on pre-existing untouched `!` assertions in
  render-fb116, not a regression). Light tier (`[polish]`, no `/src`/`/data`
  touched). — refs: BACKLOG.md fb133 Log.
...
- **2026-09-20 (scheduled routine) — fb133 ratchet shrunk 137 → 121.**
  ...
  surrounding loop), `tests/p5c-milestone-specials.test.ts` (guards
  `patches[0]` before use), `tests/p9b-codex-hub.test.ts` (guards
  `collections[0]`, twice), `tests/p9h-armour-floor-display.test.ts` (guards
  `[...enemyByKey.values()][0]`, twice), `tests/practice.test.ts` (guards
  `alive[0]` and `alive[alive.length - 1]`), `tests/progress.test.ts`
  (guards `boss[0]`, `?? 0` on `riftTimes[0]` arithmetic), `tests/q2-input-
  fuzz.test.ts` (`?? ''` on a regex capture group before `.matchAll`, twice),
  `tests/t2-selection.test.ts` (a variable-level `[number, number][]`
  tuple annotation instead of an `as`-cast, on code-reviewer's Minor),
  `tests/terrain-verify.test.ts` (guards both swapped tile reads before the
  swap), `tests/ui-fb093-ultrawide-narrow-audit.test.ts` (guards
  `present[i]`/`present[j]` before indexing `rects`), `tests/ui-fb142-dpr-
  change.test.ts` (guards `resolutionQueries[0]`), `tests/ui-fb148-dash-
  range-live.test.ts` (`?? ''` on a `.split('//')[0]` read), `tests/x001-
  dot-stack-caps.test.ts` (`?? e.hp`/`?? 0` on reads proven in-range by a
  1:1 array map), `tools/fuzz-save.ts` (`setAt`/`delAt` guard their path-tail
  key before indexing, both dead branches given the preceding `path.length
  === 0` early return), `tools/m20d-swarm.ts` (a `[number, number]` tuple
  annotation on `levels`). Verified: `npx tsc --noEmit -p
  tsconfig.unchecked.json` no longer flags any of the 16; main `tsc
  --noEmit` clean; targeted `npx vitest run` on all 16 plus the ratchet test
  green (two pre-existing `tests/q2-input-fuzz.test.ts` failures — a missing
  `place_core` Command-union member and a seed-2 tick-count flake —
  confirmed present on unmodified HEAD via `git stash`, not regressions);
  `npm run test:fast` unchanged at 315 files / 4548 passed / 35 skipped.
  code-reviewer APPROVE (one Minor fixed before commit: `t2-selection.test.ts`'s
  `as`-cast swapped for a checked tuple annotation; one Nit on pre-existing,
  out-of-scope `!` left as-is). Light tier (`[polish]`, no `/src`/`/data`
  touched) — no qa-playtester dispatch. — refs: BACKLOG.md fb133 Log.
- **2026-09-20 (scheduled routine) — fb133 ratchet shrunk 147 → 137.**
...
- **2026-09-18 — main lane: BACKLOG fb199 done — root cause is two commits
  ...
  `tests/a4-single-type.test.ts` (QUESTIONS Q210's own verification catch)
  had drifted further since fb092: `{arrow_spire:5,ballista:5,
  ember_brazier:4,frost_obelisk:0,tesla_coil:3,mortar:3,venom_spore:3}`
  against its uniform `>=4` floor. First-pass bisection of the
  `2a30281..HEAD` window attributed the whole delta to `4ce6b22` (fb153b:
  `this.gates` grew from a stale 3-gate slice to the real 4-gate list,
  which practice-mode's `flatTerrain(this.gates)`, `src/sim/world.ts`,
  builds the flat T1/T3 probe arena from) by comparing `4ce6b22^` straight
  to HEAD — **qa-playtester's verification pass caught this overclaiming a
  single cause**: an isolated control directly either side of `4ce6b22`
  shows only ember_brazier/mortar move there (`{5,5,0,0,4,4,2}` ->
  `{5,5,4,0,4,2,2}`), while `4ce6b22` -> HEAD moves tesla_coil/mortar/
  venom_spore further with no further `this.gates` change in that range —
  likely fb128's `tickCooldown` sub-tick fix (this file's own 2026-09-17
  entry already documents it flipping pinned-seed outcomes elsewhere),
  plausible but not independently isolated the way `4ce6b22` was. Zero
  `data/towers.json` commits exist either way. Neither fix is wrong (fb153b
  restored a genuinely dropped gate; fb128 fixed a real rounding bug) — the
  probe was just never designed to hold gate count or cooldown timing
  constant. Per fb199's own either/or acceptance, corrected the pin
  regardless of exact attribution rather than chasing a `data/towers.json`
  retune (already ruled out unnecessary by p12i/Q195): a per-tower
  `T1_IDENTITY_FLOOR` replaces the uniform floor, pinned to today's honest
  reading. `frost_obelisk`'s floor of 0 is named as a separate exception —
  qa-playtester traced it to at or before fb092's own commit, predating
  this item's window entirely, so its floor is inherited history, not this
  item's finding, and can't itself catch further regression; a
  from-scratch investigation of it is real, unclaimed follow-up work.
  Also corrected `tests/p11d-g13-t3-margin.test.ts`'s stale "measured
  17/18" docstring (now `waves:2, defeat_core`; its `<18` tolerance already
  covered the drift without a code change). Targeted tests green (`tests/
  a4-single-type.test.ts` full run, `tests/p11d-g13-t3-margin.test.ts`);
  `npm run test:fast` unaffected (both files are fast-tier-excluded; the
  one failure that session's own full run hit, `tests/terrain-cost.test.ts`,
  confirmed pre-existing/unrelated via a `git stash` control). Full-tier
  code-reviewer (APPROVE, two Minor/Nit notes addressed) and qa-playtester
  per CLAUDE.md's [bug] tiering — qa-playtester's own pass is what drove
  the two-commit correction above, re-verified green after. — refs:
  SPEC-FINAL §14 G13, BACKLOG fb199, p11d, p12h, p12i, QUESTIONS Q195,
  Q210, Q212.
