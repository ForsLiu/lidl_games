...
import { defaultCoreKey, loadContent } from '../src/sim/content';
import {
  SAVE_KEY,
  SAVE_VERSION,
  accountLevelFor,
...
export function validMeta(rng: Rng): MetaState {
  ...
  return {
    ...
    // SPEC-FINAL §5.5 (merge port): `MetaState` gained `unlockedCores`. The
    // default core leads the list, mirroring `allocated`'s node 0 — `migrate`
    // unshifts it when missing, so a save without it would not round-trip to
    // itself and every equality assertion downstream would be testing the
    // repair path instead of the wire format.
    unlockedCores: [
      ...new Set([defaultCoreKey(c), ...c.cores.cores.filter(() => rng.chance(0.5)).map((k) => k.key)]),
    ],
...
export function checkMeta(m: unknown): string[] {
  ...
  for (const key of ['completedQuests', 'unlockedClasses', 'unlockedCores'] as const) {
