...
const candidates: [string, number][] = [['bloodlord', 1], ['bloodlord', 10], ['bloodlord', 11], ['archer', 8], ['swordsman', 6], ['swordsman', 10], ['pyromancer', 11], ['pyromancer', 12]];
for (const [classKey, seed] of candidates) {
  const config: RunConfig = { seed, classKey, tier: GATE_TIER, modifiers: [], allocated: FULL_TREE, policy: 'hybrid', cycles: 6 };
  const withShipped = runScriptedWithContent(config, shipped);
  const withNeutral = runScriptedWithContent(config, neutral);
  console.log(classKey, seed, 'shipped:', withShipped.outcome, 'w'+withShipped.wavesCleared, '| neutral:', withNeutral.outcome, 'w'+withNeutral.wavesCleared, withShipped.outcome === 'defeat_warden' && withNeutral.outcome !== 'defeat_warden' ? '<<< DISCRIMINATES' : '');
}
