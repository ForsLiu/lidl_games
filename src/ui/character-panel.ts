...
function sourceLabel(w: World, source: StatSource): string {
  ...
  switch (prefix) {
    case 'class': {
      const id = parts[1] ?? '';
      const cls = w.content.classByKey.get(id);
    ...
    case 'boon': {
      const id = parts[1] ?? '';
      const def = w.content.boonByKey.get(id);
      const rank = w.boonRanks[id] ?? 0;
      return `Boon: ${def?.name ?? id} (rank ${rank})`;
    }
    case 'core': {
      const id = parts[1] ?? '';
      const def = w.content.coreByKey.get(id);
    ...
    case 'equipment': {
      ...
      const id = parts[1] ?? '';
      const item = w.content.equipmentByKey.get(id);
...
export function characterPanelData(w: World): CharacterPanelData {
  ...
  const boons: BoonRow[] = Object.keys(w.boonRanks)
    ...
    .map((key) => {
      ...
      // The preceding .filter() already proved w.boonRanks[key] > 0.
